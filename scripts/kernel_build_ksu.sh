...
export CHATID API_BOT MIUI KERNEL_VERSION KSU

if [ "$MIUI" == yes  ];
then
DEVICE="REDMI NOTE 10 PRO & PRO MAX (MIUI)"
KERNEL_NAME="SLEEPY_KERNEL-MIUI-KSU"
CODENAME="SWEET"
# revert commit
git revert 5127bd1 --no-edit
else
DEVICE="REDMI NOTE 10 PRO & PRO MAX (OSS)"
KERNEL_NAME="SLEEPY_KERNEL-OSS-KSU"
CODENAME="SWEET"
fi
...
# clang stuff
		...
		git clone --depth=1 https://gitlab.com/itsshashanksp/android_prebuilts_clang_host_linux-x86_clang-r487747c.git  "$HOME"/clang
